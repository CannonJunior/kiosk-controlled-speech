...
class SpeechWebBridge:
    ...
    def __init__(self):
        ...
        self.screenshot_service = None  # Will be initialized after MCP client is ready
        self.enhanced_mcp_client = None  # Enhanced MCP client wrapper
    ...
    async def initialize(self):
        ...
        try:
            ...
            # Initialize enhanced MCP client wrapper
            self.enhanced_mcp_client = EnhancedMCPClient()
            await self.enhanced_mcp_client.initialize()
            
            # Initialize text reading service
            self.text_reading_service = TextReadingService(self.mcp_client)
            
            # Initialize annotation services
            self.screenshot_service = ScreenshotService(self.enhanced_mcp_client, metrics_collector)
            
            # Initialize dependencies for API routes
            initialize_dependencies(self.enhanced_mcp_client)
    ...
    async def _execute_suggested_action(self, ollama_response: Dict[str, Any], current_screen: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if action_type == "click":
                ...
                # Execute screenshot through annotation service
                try:
                    if self.screenshot_service:
                        result = await self.screenshot_service.capture_screenshot()
                        ...
                        return {
                            ...
                            "message": "❌ Screenshot service not initialized"
                        }
                except Exception as e:
    ...
    async def _execute_fast_path_action(self, response_data: Dict[str, Any], original_message: str, current_screen: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action_type == "help":
            ...
            # Execute native screenshot through annotation service (portal command)
            try:
                if self.screenshot_service:
                    result = await self.screenshot_service.capture_screenshot()
                    action_result = result.to_dict()
                    
                    # Format as portal-style response
                    if result.success:
                        action_result["message"] = f"🚀 **Portal Screenshot Captured!**\n📁 File: {result.screenshot.filename}\n📏 Size: {result.screenshot.size_bytes}\n🖼️ Dimensions: {result.screenshot.dimensions.get('width', 0)}x{result.screenshot.dimensions.get('height', 0)}\n⚙️ Method: {result.screenshot.method}\n\n✨ Native MCP screenshot capture completed!"
                    else:
                        action_result["message"] = f"❌ **Portal Screenshot Failed**\n\nError: {result.error}\n\nTry: 'portal screenshot' again or use 'take screenshot'"
            ...
            # Execute screenshot through annotation service (fast-path)
            try:
                if self.screenshot_service:
                    result = await self.screenshot_service.capture_screenshot()
                    action_result = result.to_dict()
                    
                    # Format as fast-path response
                    if result.success:
                        action_result["message"] = f"📸 Fast-path screenshot captured! File: {result.screenshot.filename}"
                    else:
                        action_result["message"] = f"❌ Fast-path screenshot failed: {result.error}"
                    ...
                    action_result = {
                        ...
                        "action_type": "screenshot",
                        "error": "Screenshot service not available",
                        "message": "❌ Fast-path screenshot error: Screenshot service not initialized"
                    }
...
async def startup_event():
    ...
    try:
        ...
        # Configure communication domain handlers
        text_reading_service = TextReadingService(speech_bridge.mcp_client)
        communication_integration.configure_communication_handlers(
            speech_bridge, 
            text_reading_service
        )
...
async def websocket_endpoint(websocket: WebSocket, client_id: str):
    """WebSocket endpoint for real-time communication (original implementation)"""
...
async def websocket_endpoint_v2(websocket: WebSocket, client_id: str):
    """WebSocket endpoint using Communication Domain (new implementation)"""
    await communication_integration.handle_websocket_connection(websocket, client_id)
...
# Annotation routes moved to /api/v1/annotation/ endpoints via annotation_router
